...
class Mouse:
    ...
    def __init__(self, mouse, client, labels_row=2, spreadsheet_name='sorting steps', **kwargs):
        ...
        #set the client to point at the cells feed corresp. to the worksheet of this mouse
        try:
            self.workSheet = spreadsheets.Spreadsheets(client)
            self.workSheet.get_spreadsheet(spreadsheet_name)
            self.workSheet.get_worksheet(mouse)

        except:
            print "Spreadsheet for mouse " + self.mouse + "not found!"
         
        self.labelsRow=labels_row
        self.labels=self.workSheet.get_row(self.labelsRow)
    ...
    def get_sessions(self):
        ...
        sessionsList=self.workSheet.get_col(sessionsCol,row_range= [self.labelsRow+1,'end'])
    ...
    def get_recs(self, rowStart, rowEnd, **sess):
        row= rowStart
        nRec=0
        recs=[]
        recsCol=self.get_label_col('rec')
        recList=self.workSheet.get_col(recsCol,col_range=[row,'end'])
    ...
    def get_units(self,**kwargs):
        ...
        for icell in range(row-1,rowEnd):
            ...
            if unitListString is not None:
                ...
                for propKey, propElem in unitProps.items():
                    readVal  = self.workSheet.cell(icell+1,propElem[3]).value
                    propType = propElem[0]
