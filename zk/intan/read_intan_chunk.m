function intanChunk=read_intan_chunk(fileName,beQuiet)
...
% GUI.  Data are parsed and placed into variables that appear in the structure intanChunk.
%
% Example:
% >> clear
...
if ~exist('beQuiet') || isempty('beQuiet')
    beQuiet=0;
end
...
if ~beQuiet
    fprintf(1, 'Reading Intan chunk %s \n', fileName);
end
...
if (data_present)
    ...
    for i=1:num_data_blocks
        ...
        if (fraction_done >= percent_done) && ~beQuiet
            fprintf(1, '%d%% ...', percent_done);
...
move_to_return_structure(notes);
move_to_return_structure(frequency_parameters);

move_to_return_structure(num_amplifier_channels);
if (num_amplifier_channels > 0)
    move_to_return_structure(amplifier_channels);
    if (data_present)
        move_to_return_structure(amplifier_data);
        move_to_return_structure(t_amplifier);
    end
    move_to_return_structure(spike_triggers);
end
move_to_return_structure(num_aux_input_channels);
if (num_aux_input_channels > 0)
    move_to_return_structure(aux_input_channels);
    if (data_present)
        move_to_return_structure(aux_input_data);
        move_to_return_structure(t_aux_input);
    end
end
move_to_return_structure(num_supply_voltage_channels);
if (num_supply_voltage_channels > 0)
    move_to_return_structure(supply_voltage_channels);
    if (data_present)
        move_to_return_structure(supply_voltage_data);
        move_to_return_structure(t_supply_voltage);
    end
end
move_to_return_structure(num_board_adc_channels);
if (num_board_adc_channels > 0)
    move_to_return_structure(board_adc_channels);
    if (data_present)
        move_to_return_structure(board_adc_data);
        move_to_return_structure(t_board_adc);
    end
end
move_to_return_structure(num_board_dig_in_channels);
if (num_board_dig_in_channels > 0)
    move_to_return_structure(board_dig_in_channels);
    if (data_present)
        move_to_return_structure(board_dig_in_data);
        move_to_return_structure(t_dig);
    end
end
move_to_return_structure(num_board_dig_out_channels);
if (num_board_dig_out_channels > 0)
    move_to_return_structure(board_dig_out_channels);
    if (data_present)
        move_to_return_structure(board_dig_out_data);
        move_to_return_structure(t_dig);
    end
end
move_to_return_structure(num_temp_sensor_channels);
if (num_temp_sensor_channels > 0)
    if (data_present)
        move_to_return_structure(temp_sensor_data);
        move_to_return_structure(t_temp_sensor);
    end
end

if ~beQuiet
    fprintf(1, 'Done!  ( %0.1f s)\t', toc);
end
if (data_present)
   if ~beQuiet 
       fprintf(1, 'Data available.\n');
   end
else
    fprintf(1, 'No data.\n');
end

    function move_to_return_structure(variable)
        
        % move_to_return_structure(variable)
        %
        ...
        intanChunk.(variable_name)=variable;
