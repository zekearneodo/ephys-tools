...
function [trial_numbers, events_list] = make_tables(mouse,sess,rec,run)
...
input_par = struct('par',[],'default',[],'validation',[]);
...
% trial pin
% trial pin is special (weird)
% it is the first event in the list and it needs to be extracted before all
% the others.
% First, trialNumbers are matched
% second, make_trial_table reshapes the output of trialNumbers
% But for now (april 2015) I just leave it as
ev.name     = 'trialPin';
ev.type     = 'digital';
ev.chanId   = 'trPin';
ev.evtFcn   = @match_trial_numbers;
ev.tableFcn = @make_trial_table;
...
trial_numbers = match_trial_numbers(mouse,sess,rec,run);
...
%events_list(1).table = make_trial_table(ev,trial_numbers,mouse,sess,rec,run);

for ie = 1:numel(events_list);
    ev = events_list(ie);
    events_list(ie).table = events_lookup(ev,trial_numbers,mouse,sess,rec,run);
end
...
function to=make_laser_table(ev,events,evTrials,mouse,sess,rec,irun)
 ...
 % go through the trials, check the trial has an open valve, and fill the
 % table.
 [raw_trials, raw_table] = read_voyeur_fields(ev.name, mouse,sess,rec,irun);
 %filter only the trials that come with clean corresponding events from
 %evTrials
 trials = [];
 tolerance = 0.15;
 ...
 for iEv = 1:length(evTrials)
     thisTrial    = raw_trials([raw_trials.trialNumber]==evTrials{iEv,1});
     thisEventIdx = evTrials{iEv,2};
 ...
 [rec_info, ~] = get_info(mouse,sess,rec,irun);
 laser_chan = rec_info.chan(strcmpi(ev.chanId,{rec_info.chan.name}));
 
 %if the channel has a gain defined, it calculated the expected values and
 %they are in trials.v
 if isfield(laser_chan,'gain') && ~isempty(laser_chan.gain) && laser_chan.gain>0
...
if strcmpi(inPar.Results.aqSystem,'JFRC')
    ...
    chanGain = 1;
    if isfield(eventChan,'gain') && ~isempty(eventChan.gain) && eventChan.gain > 0
...
for jf = 1:numel(fieldsList)
    ...
    elseif ischar(table.(fieldName))
        ...
        newCol = [];
        for il = 1:size(table.(fieldName),2)
