...
function vr = visualize_responses(mouse,sess,rec,unit,sType)
global vp vd dm;  
...
%     rmax=numel(stim(ks).in_tr);
    ...
    if strcmp(vp.mouse,'ZKawakeM72') & str2num(vp.sess)==1 & strcmp(vp.rec,'g')
        ...
        %select trials for et tiglate.
        stim(1).in_tr=stim(1).in_tr(end-upperTrials+1:end);
    end
    
    if strcmp(vp.mouse,'SDbehavingM72') & str2num(vp.sess)==3 & strcmp(vp.rec,'c') & strcmp(sType,'odor')
        vp.stim1=stim(1);
        disp(numel(stim(1).in_tr));
        if ks==1
            stim(1).in_tr=stim(1).in_tr([29:44 51:58 67:77]);
        elseif ks==2
        stim(2).in_tr=stim(2).in_tr([14:26 32:34 36:end]);
        end
    end
