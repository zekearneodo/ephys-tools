...
function vr = visualize_responses_sd(mouse,sess,rec,unit,sType)
global vp vd dm;  
...
if strcmp(sType,'odor')
    ...
    for kt=1:numel(tr)
        ...
        if isempty(iiBase) || iiBase<1 || isempty(trial.spikeTimes)
            continue
        end
...
%     rmax=numel(stim(ks).in_tr);
    ...
    if strcmp(vp.mouse,'ZKawakeM72') & str2num(vp.sess)==1 & strcmp(vp.rec,'g')
        upperTrials=min(20,numel(stim(ks).in_tr));
        stim(ks).in_tr=stim(ks).in_tr(end-upperTrials+1:end);
    end
    
    if strcmp(vp.mouse,'SDbehavingM72') & str2num(vp.sess)==3 & strcmp(vp.rec,'c')
        upperTrials=min(42,numel(stim(ks).in_tr));
        stim(ks).in_tr=stim(ks).in_tr(end-upperTrials+1:end);
    end
    ...
    for it = stim(ks).in_tr
        ...
        if stim(ks).odorConc >0 & strcmp(sType,'odor')
            ...
            %get the time of first lick relative to sniff onset
            if(isfield(tr(it).VoyeurParameters,'inh_onset'))
                if(tr(it).VoyeurParameters.result==1 || tr(it).VoyeurParameters.result==3)
                    firstLick(it) = tr(it).VoyeurParameters.firstlick-tr(it).VoyeurParameters.inh_onset;
                    if(firstLick(it)<0)
                        firstLick(it)=0;
                    end
                end
