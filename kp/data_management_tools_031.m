...
%   %OBSOLETE as of Feb 13, 2014. replaced by data_management_tools_032.m
%   in zk folder of the ephyxDataManagement of the git repository.
...
function dm = data_management_tools_031()
global dm
...
%     
    ...
    fn.fold_analysis  = fullfile(fn.disk, 'analysis');
...
%     fn.fold_sd_data  = fullfile(stat_disk, '');
    if nargin > 1
        ...
        fn.fold_an_mouse  = fullfile(fn.fold_analysis, sprintf('mouse_%s', mouse));
...
%global info
    ...
    for irec = 1:numel(reclist)
        ...
        sf(irec)=figure(), clf
        title(sprintf('Shift estim, rec %s',info.rec(nrec).name), 'FontSize', 10, 'FontWeight', 'bold')
...
%             plot(eSyncRun,ones(size(eSyncRun))*15000,'ro')
            ...
            % beh trigger (discard the trials with null duration)
...
%                 
                if error(it)<err_min
                    ...
                    figure(runFig(irun))
                    clf
                    plot(eTimes,ones(size(eTimes))*10000,'r*')
                    hold on
                    plot(bTimes*(alpha(it)),ones(size(eTimes))*15000,'mx')
                    plot(eTRun(eT0:end))
                    title(sprintf('Shift estim + align, run %02d',irun), 'FontSize', 10, 'FontWeight', 'bold')
                    ...
                    trAlign.iTrialsRun = in(ind); %beh trials that have a matching ephys trial.
                end
                drawnow
            end
...
%aligns the trials and the ephys file.
    ...
    for irec = 1:numel(rec),    % ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        ...
        nrec = find(strncmpi(cellstr({q.info.rec.name}),rec(irec),1))
