...
for ic = 1:nCells
        ...
        if otr<=numel(trialsBase)
            ...
            if ~strcmp(trialUId,trialsBase(otr).trialId)
                ...
                if ~isempty(trial(itr).spikeTimes)
                    ...
                    spikeTimes = allspikes(allspikes>trialsBase(otr).start...
                        & allspikes<=(trialsBase(otr).start+numel(trialsBase(otr).sniffPhase)))...
                        - trialsBase(otr).start;
    ...
    figure; imagesc(spikes); colormap('gray')
    figure; imagesc(snPhase); colormap('gray')
    end
...
%_______________________________________________________________________%
    function theCell=getCell(unit_filename)
        ...
        if theCell.quality==1 && theCell.light==0 && theCell.odor==1
