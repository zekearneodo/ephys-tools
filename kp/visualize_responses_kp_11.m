...
if isfield(vp.tr,'pulseOnsetDelay')
    vp.par = {'odorName', 'odorConc', 'laserDur', 'laserAmp', 'pulseOnsetDelay'}
...
if isfield(vp.tr,'pulseOnsetDelay')
    stim = struct('odorName', '', 'odorConc', 0, 'laserDur', 0, 'laserAmp', 0, 'in_tr', [], 'pulseOnsetDelay', [])
    tr=vp.tr;
elseif isfield(vp.tr,'pulseOffset')
    stim = struct('odorName', '', 'odorConc', 0, 'laserDur', 0, 'laserAmp', 0, 'in_tr', [], 'pulseOffset', [],'pulseGroup',[])
    tr=vp.tr;
else
...
if isfield(stim,'pulseOffset')                             % for KPawakeM72 sess_004 only
...
% Get baseline rate for all odor trials
%   by averaging sime size window around inhalation onset 
%   for the 3 sniffs before final valve opening each trial
...
%     stim(ks).in_tr=stim(ks).in_tr(1:upperTrials);
    ...
    x = x(1:nsp);
    y = y(1:nsp);
    spkBaseline = sum(rate([t1:t2]<0))*(nt/1000)/kt
...
%     isi_thr = 1/90*1000;
    ...
    for jt = 1:kt
        ...
        ISI(jt).resp_spk_t = underthr(underthr > 0-t1 & underthr < respwin-t1);
...
%     ymax(2+max(hh))
    ...
    plot(x,y, '.g', 'MarkerSize',9),
        plot(xx,yy, '.', 'MarkerSize',9), %spikes with ISI below threshold
