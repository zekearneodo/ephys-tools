...
% outputs vr (function sub-routines) and the response struct created
...
% -view_lfp (if surface electrode)

function [vr, resp] = visualize_responses_lfp(mouse,sess,rec,unit,sType)
global vp vd;  
...
chan = 11;
trialsPlot = 1:5;
% view_lfp(sType,chan,trialsPlot)
...
switch vp.tr_type
    case 1
        stim = struct('odorName', '', 'odorConc', 0, 'laserDur', 0, 'laserAmp', 0, 'in_tr', [], 'stim_str', '');
    case 2
        stim = struct('odorName', '', 'odorConc', 0, 'laserDur', 0, 'laserPower', 0, 'in_tr', [], 'pulseOffset', [],'pulseGroup',[], 'stim_str', '');
    case 3
        stim = struct('odorName', '', 'odorConc', 0, 'laserDur', 0, 'laserPower', 0, 'in_tr', [], 'pulseOnsetDelay', [], 'stim_str', '');
end
...
rateBase=zeros(1,nt);
cumulativeCountBase = zeros(1e3,nt);
...
cumulativeCountBase = cumulativeCountBase(1:nb,:);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    spkCounts_trials=[];    cumulativeCount = zeros(1e3,nt);
...
%         sp_cC = sp(sp>0);   
    ...
    % Finished collecting spike data for each trial, so now trim raster,
    % reshape histogram vector, and calculate response statistics
    x = x(1:nsp);
    y = y(1:nsp);
...
tPlot=zeros(numel(trialsPlot),2);
tVec=(t1+1:t2)*19385;

%%%
%%% Get the list of channels and find the one you want to work with
%%%


q = load(fn.ss_sess_info);
% recNumber = find(strcmpi({vp.rec},{q.info.rec.name}));
rec = q.info.rec(strcmpi({vp.rec},{q.info.rec.name}));

if chan<9 
    chan_name = sprintf('S0%d',chan);
elseif chan>9
    chan_name = sprintf('S%d',chan);
elseif chan==0
    error('Channels indexed from 1');
end

chanNumber = find(strcmpi(chan_name,{rec.chan.name}));


%%%
%%% Get the channel
%%%
lfp_filepath=fullfile(vp.fn.fold_ss_rec,'rec_b.lfp');
lfp_file = fopen(lfp_filepath, 'r');

chanData = read_analog_channel(lfp_file, chanNumber, rec.nChan);
%  X_norm = X - mean(X);
...
%             stim_str = sprintf('Laser: \n%2.1fmW -%3.1fms', stim(ks).laserPower, stim(ks).laserDur);
            stim_str = stim(ks).stim_str;
        end
        kt = kt + 1;
...
end     %view_lfp(sType,chan,trialsPlot)

function X = read_analog_channel(fid, ich, nch)
        fseek(fid, 2*(ich-1), 'bof');
        disp(sprintf('  ...Reading ch %d of lfp file... ',ich))
        X = fread(fid, inf, 'int16', (nch-1)*2);
end