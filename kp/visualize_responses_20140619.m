...
sniffLengths   = zeros(numel(tr),1);
inhLengthsStim = zeros(numel(tr),2);
for kt = 1:numel(tr)
    trial=tr(kt);
    ...
    while oneMoreWindow 
        ...
        if nWindow==0   
            ...
            sniffLengths(kt,:)   = diff(trial.sniffZeroTimes(1,iFirstSniff:iFirstSniff+1));
            inhLengthsStim(kt,:) = diff(trial.sniffZeroTimes(:,iFirstSniff:iFirstSniff+1));
