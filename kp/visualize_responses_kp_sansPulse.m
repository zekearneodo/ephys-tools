...
% uses data_managment_tools_031
% % 
% Updated by kp, 2014-03-04
%   Same as most recent, "visualize_responses_kp.m" but used for preps 
%   that do not have .pulseOffset field in trial structure

function vr = visualize_responses_kp_sansPulse(mouse,sess,rec,unit,sType)
global vp vd dm;  
...
%     rmax=numel(stim(ks).in_tr);
    ...
    for it = stim(ks).in_tr
        ...
        rate(sp-t1) = rate(sp-t1) + 1; %if a spike occurred in that ms in current trial, add one
        n  = numel(sp);
