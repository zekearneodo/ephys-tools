...
function [vr, resp] = visualize_responses(mouse,sess,rec,unit,sType)
global vp vd;
...
vp.smooth.stdev = 19385/(2*pi*vp.smooth.cutoff); %std for gaussian smoothing of histo for plotting
% q.info.rec(nrec).sampling_freq
q = load(vp.fn.trial);
...
% % %       
    ...
    for ks = 1:numel(stim)
        ...
        % Response Window is time after stim you count n spikes
        ...
        spkCountBase   = sum(rateBase(tms>0 & tms<responseWindow)/1000);
        ...
        FULLpreStim_resp_times=zeros(2e2,10);
        FULLpostStim_resp_times=zeros(2e2,40);
...
%             end
    ...
    %%%% Build matrix of respiration times, referenced to t0
                ...
                if numel(keepthese)>size(FULLpreStim_resp_times,2)
                    keepthese=keepthese(end+1-size(FULLpreStim_resp_times,2):end);
                end
                FULLpreStim_resp_times(kt,(end+1-numel(keepthese)):end)=keepthese;

                for iph=1:numel(postphases)-1
                    FULLpostStim_resp_times(kt,2*iph-1:2*iph)=tr(it).sniffZeroTimes(:,postphases(iph))'-t0;
                end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            if n > 1
                ISI_raw(kt,sp(2:end)-t1) = diff(sp);  % vector of ISIs
                ISI_raw(kt,sp(1)-t1) = inf;
...
%         cumulativeCount = cumulativeCount(1:kt,:);
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %%%%% Create vectors for filling inhalation periods grey and 
        %%%%%  sort trials in raster by length of inhalation
        
        resp_t0 = size(FULLpreStim_resp_times,2)+1;
        respirationTimes = [FULLpreStim_resp_times FULLpostStim_resp_times];
        respirationTimes = respirationTimes(1:kt,:);
        
        [inh1_length,inh1_idx]=sort(respirationTimes(:,resp_t0+1));

        if ~vp.sort
            inh_plot_x = [respirationTimes([1 1:end end],resp_t0+1)' 0 respirationTimes(:,resp_t0)'];
            inh_plot_y = [0:kt kt kt:-1:0];
            
        elseif vp.sort
            inh_plot_x = [inh1_length([1 1:end end])' 0 respirationTimes(:,resp_t0)'];
            inh_plot_y = [0:kt kt kt:-1:0];
...
%         latency_cumulativeCount = find(diff(diff_combinedCount),1) + t1;
    ...
    %%%% Plot raster in upper subplot
        ...
        if ~vp.sort
            ...
            plot(firstISI',1:kt, 'g.', 'MarkerSize',6)
        elseif vp.sort
...
% function resp = view_warped_rasters(sType,un)
% end
